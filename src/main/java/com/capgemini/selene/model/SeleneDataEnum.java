...
public enum SeleneDataEnum {
	...
	SeleneDataEnum(String name, Kind kind, Unit unit, float min, float max) {
		this(name,kind, unit, "", min, max);
	}

	SeleneDataEnum(String name, Kind kind, Unit unit, String chemicalElement, float min, float max) {
		this.name = name;
		this.kind = kind;
		this.unit = unit;
